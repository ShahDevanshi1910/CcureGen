...
public class SnortRules extends AppCompatActivity {
    ...
    private static final String IP ="172.16.12.96";

    private static String JSON_URL = null;

    ArrayList<String> list;
//    To add list value to spinner
...
//Portrait remains the same
        this.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);

        list = new ArrayList<String>();
//        Listview item is listed in spinner format
...
//Try catch for handling exception , data of selected ip : meta data
                    try {
    ...
    private void LoadData() {
        ...
        StringRequest str = new StringRequest(Request.Method.GET, JSON_URL, new Response.Listener<String>() {
            @Override
            //Try catch to catch the error if exist, get data
            public void onResponse(String response) {
                try {
                    JSONObject jobj = new JSONObject(response);
                    JSONArray dataarray = jobj.getJSONArray("folders");
                    for(int i=0;i< dataarray.length();i++){
                        //IP's are added to the list
